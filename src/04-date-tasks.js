...
function timeSpanToString(startDate, endDate) {
  ...
  const hours = Math.floor(diffDate / (60 * 60 * 1000));
  diffDate -= hours * 3600000;
  const minutes = Math.floor(diffDate / (60 * 1000));
  diffDate -= minutes * 60000;
  const seconds = Math.floor(diffDate / 1000);
  diffDate -= seconds * 1000;
  const mSeconds = diffDate;
  return `${String(hours).padStart(2, 0)}:${String(minutes).padStart(2, 0)}:${String(seconds).padStart(2, 0)}.${String(mSeconds).padStart(3, 0)}`;
}
